...
import com.teamadhoc.wifigroupstream.Player.ServerDeviceListFragment.PlayerFragmentListener;
public class PlayerActivity extends Activity implements WifiP2pManager.ChannelListener,
        PlayerFragmentListener { // PlayerAc
    // tivity에 implements WifiP2pManager.ChannelListener,PlayerFragmentListener를 이용한 경우
    public final static int Player_MODE = 15; // 그룹 소유자 인 가장 높은 성향을 나타냄, 클래스에서 사용할 해당 멤버 변수의 데이터와 그 의미, 용도를 고정
    ...
    public void resetDeviceList() {
        ServerDeviceListFragment fragmentList = (ServerDeviceListFragment) getFragmentManager()
                .findFragmentById(R.id.frag_Player_devices);
    ...
    public void connect(WifiP2pConfig config) {
        ...
        // Player 모드에서는 그룹 소유자가되고 싶습니다
        // In Player mode, we want to become the group owner
        WifiP2pConfig newConfig = config;
        newConfig.groupOwnerIntent = Player_MODE; //그룹 소유자 인 가장 높은 성향을 나타냅 (Indicates the highest inclination to be a group owner)
    ...
    public void disconnect() {
        ...
        manager.removeGroup(channel, new WifiP2pManager.ActionListener() {
            ...
            @Override
            public void onSuccess() {
                Toast.makeText(PlayerActivity.this, "Disconnected", Toast.LENGTH_SHORT).show();
